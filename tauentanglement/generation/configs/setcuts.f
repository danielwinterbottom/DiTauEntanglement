...
C
      ...
      LOGICAL  IS_A_PI(NEXTERNAL)
      logical  do_cuts(nexternal)
      COMMON /TO_SPECISA/IS_A_J,IS_A_A,IS_A_L,IS_A_B,IS_A_NU,IS_HEAVY,
     . IS_A_ONIUM, do_cuts, IS_A_PI 
...
c-charged-leptons
         ...
         if (abs(idup(i,1,iproc)).eq.111)  is_a_pi(i)=.true. ! pi0
         if (abs(idup(i,1,iproc)).eq.211)  is_a_pi(i)=.true. ! pi+ ti-
c-b-quarks
...
c     detect if mmnl is used for exactly 2 lepton      
      if (mmnl.ne.0.or.mmnlmax.ne.-1)then
          ncheck = 0
          do i=nincoming+1,nexternal
            if (is_a_l(i).or.is_a_nu(i).or.is_a_pi(i)) ncheck = ncheck +1
          enddo 
          ! exactly two lepton/neutrino put constrain on the invariant
          ! mass of the associated particles
          if (ncheck.eq.2) then
              do i =nincoming+1, nexternal-1
                 if (is_a_l(i).or.is_a_nu(i).or.is_a_pi(i)) then
                     do j = i+1, nexternal
                        if (is_a_l(j).or.is_a_nu(j).or.is_a_pi(j)) then
